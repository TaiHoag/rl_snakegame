...
LEARNING_RATE = 0.0001  # Further reduced for stability with new architecture
EPSILON = 1.0
...
SAVE_PATH = os.path.join(SAVE_DIR, "modelv3.pt")
UPDATE_TARGET_EVERY = 3  # More frequent target network updates

class GNNStateEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        super(GNNStateEncoder, self).__init__()
...
class DQNetwork(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.gnn = GNNStateEncoder(input_dim=4, hidden_dim=128)  # Increased GNN dimension
        
        # Calculate total input dimension
        self.total_input_dim = state_dim  # Basic state (12) + GNN features (128)
        
        # Enhanced network architecture with skip connections and layer normalization
    ...
    def forward(self, state, graph_data=None):
        ...
        if graph_data is not None:
            gnn_features = self.gnn(graph_data)  # Shape: [128]
            if len(gnn_features.shape) == 1:
                gnn_features = gnn_features.unsqueeze(0)  # Shape: [1, 128]
            gnn_features = gnn_features.expand(batch_size, -1)  # Shape: [batch_size, 128]
            
            # Ensure proper concatenation
            if state.size(1) != 12:  # Verify state dimension
                raise ValueError(f"Unexpected state dimension: {state.size()}")
            x = torch.cat([state, gnn_features], dim=1)  # Shape: [batch_size, 12 + 128]
        else:
            x = state
        ...
        h2 = F.relu(self.ln2(self.fc2(h1) + self.skip1(h1)))
        h2 = self.dropout(h2)
        
        h3 = F.relu(self.ln3(self.fc3(h2) + self.skip2(h2)))
        h3 = self.dropout(h3)
        
        # Dueling DQN: Split into value and advantage streams
        value = self.value_stream(h3)
        advantage = self.advantage_stream(h3)
